...
- Line breaks differ by language. English uses `text-wrap-style: balance` for headings and short labels and plain wrapping for paragraphs (Safari's `pretty` shortens every line of a paragraph, which looks like a narrow column on an iPhone). Chinese keeps full lines, as Chinese typesetting expects; `keepPhrasesTogether()` in `app.js` keeps each block's last two words with the closing punctuation and keeps product names such as Claude Code on one line. Do not add pretty or balance to Chinese text: it leaves ragged line ends and still splits words.
